...
public class YouTubeMediaItemService implements MediaItemService {
    ...
    private MediaItemFormatInfo selectPlaybackFormatInfo(String videoId, String clickTrackingParams) {
        ...
        if (formatInfo == null || formatInfo.isUnplayable()) {
            try {
                MediaItemFormatInfo fallback = getFormatInfoInnertube(videoId, clickTrackingParams);
                // Keep the authenticated response's restriction reason when fallback fails.
                if (formatInfo == null || (fallback != null && !fallback.isUnplayable())) {
                    formatInfo = fallback;
                }
                ...
                Log.e(TAG, "Fallback failed; preserving the original playback denial (%s)", error.getClass().getSimpleName());
            }
        }
    ...
    MediaItemFormatInfo getFormatInfoLegacy(String videoId, String clickTrackingParams) {
        checkSigned();
    ...
    MediaItemFormatInfo getFormatInfoInnertube(String videoId, String clickTrackingParams) {
        checkSigned();
        return InnertubeService.createFormatInfo(videoId);
    }
    ...
    private MediaItemFormatInfo getCachedFormatInfo(String videoId) {
        return  mCachedFormatInfo != null && !mCachedFormatInfo.isUnplayable() &&
