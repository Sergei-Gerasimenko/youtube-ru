...
import com.liskovsoft.youtubeapi.app.playerdata.PlayerUrlResolver;
...
public class AppService {
    ...
    private PlayerDataExtractor mTvPlayerDataExtractor;
    ...
    public Pair<List<String>, List<String>> bulkSigExtract(List<String> nParams, List<String> sParams) {
        return bulkSigExtract(nParams, sParams, null);
    }

    public Pair<List<String>, List<String>> bulkSigExtract(List<String> nParams, List<String> sParams, AppClient client) {
        if (Helpers.allNulls(nParams, sParams)) {
            return null;
        }

        PlayerDataExtractor extractor = getPlayerDataExtractor(client);
        return extractor != null ? extractor.bulkSigExtract(nParams, sParams) : null;
    }

    private synchronized PlayerDataExtractor getPlayerDataExtractor(AppClient client) {
        if (client == null || !client.isTVClient()) {
            return mAppServiceInt.getPlayerDataExtractor();
        }

        String playerUrl = PlayerUrlResolver.resolve(mAppServiceInt.getPlayerUrl(), client);
        if (playerUrl == null) {
            return null;
        }
        // TV and web players need separate decoders, even with the same revision.
        if (mTvPlayerDataExtractor == null || !playerUrl.equals(mTvPlayerDataExtractor.getPlayerUrl())) {
            mTvPlayerDataExtractor = new PlayerDataExtractor(playerUrl);
        }
        return mTvPlayerDataExtractor;
    }
    ...
    public String getSignatureTimestamp(AppClient client) {
        PlayerDataExtractor extractor = getPlayerDataExtractor(client);
