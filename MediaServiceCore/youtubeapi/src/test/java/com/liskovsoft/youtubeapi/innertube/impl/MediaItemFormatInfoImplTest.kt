...
class MediaItemFormatInfoImplTest {
    ...
    @Test
    fun ageVerificationDenialCannotBeTreatedAsPlayable() {
        val response = parsePlayerResponse("player/web/2025.11.10_player_regular.json")
        val denied = response.copy(
            playabilityStatus = PlayerResult.PlayabilityStatus(
                "AGE_VERIFICATION_REQUIRED", "Verify your age", false, null
            ),
            streamingData = null
        )
        val formatInfo = MediaItemFormatInfoImpl(denied)
        assertTrue(formatInfo.isUnplayable)
        assertEquals("Verify your age", formatInfo.playabilityReason)
    }
